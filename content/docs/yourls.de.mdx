...
Der Parameter `signature` ist Ihr **S.EE API Token**.
...
|----------|----------|-------------|
| `signature` | Yes | Ihr S.EE API Token |
| `action` | Yes | Die auszuführende Aktion (derzeit nur `shorturl`) |
| `url` | Yes | Die zu kürzende URL |
| `format` | No | Ausgabeformat: `json`, `jsonp`, `xml` oder `simple` (Standard: `simple`) |
| `keyword` | No | Benutzerdefinierter Slug für die Kurz-URL |
| `title` | No | Titel für die Kurz-URL |

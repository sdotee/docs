...
Tham số `signature` chính là **API Token S.EE** của bạn.
...
|----------|----------|-------------|
| `signature` | Yes | API Token S.EE của bạn |
| `action` | Yes | Action cần thực hiện (hiện chỉ hỗ trợ `shorturl`) |
| `url` | Yes | URL cần rút gọn |
| `format` | No | Định dạng đầu ra: `json`, `jsonp`, `xml`, hoặc `simple` (mặc định: `simple`) |
| `keyword` | No | Slug tùy chỉnh cho URL ngắn |
| `title` | No | Tiêu đề cho URL ngắn |

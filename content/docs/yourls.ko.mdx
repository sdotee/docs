...
`signature` 파라미터에는 **S.EE API Token** 을 사용합니다.

API Token 은 다음 페이지에서 생성하고 관리할 수 있습니다.

https://s.ee/user/developers/

API Token 은 인증된 사용자에 귀속되며, 계정과 동일한 권한 및 제한을 따릅니다.
...
|----------|----------|-------------|
| `signature` | Yes | S.EE API Token |
| `action` | Yes | 수행할 액션 (현재는 `shorturl` 만 지원) |
| `url` | Yes | 단축할 URL |
| `format` | No | 출력 형식: `json`, `jsonp`, `xml`, `simple` (기본값: `simple`) |
| `keyword` | No | 단축 URL 용 사용자 정의 슬러그 |
| `title` | No | 단축 URL 제목 |

...
Parameter `signature` adalah **API Token S.EE** Anda.
...
|----------|----------|-------------|
| `signature` | Yes | API Token S.EE Anda |
| `action` | Yes | Aksi yang akan dijalankan (saat ini hanya `shorturl`) |
| `url` | Yes | URL yang akan dipendekkan |
| `format` | No | Format output: `json`, `jsonp`, `xml`, atau `simple` (default: `simple`) |
| `keyword` | No | Slug kustom untuk URL pendek |
| `title` | No | Judul untuk URL pendek |
